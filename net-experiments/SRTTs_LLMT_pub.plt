...
, '<grep -P -o "(?<=K/)(\d*)(?= us )" afmt_fl/iperf_tentry.log' skip rls_to_ignore using (1.0):(column(1)/1000):(0):('AFMT') \
, '<grep -P -o "(?<=K/)(\d*)(?= us )" srtt_min_busy_wait/iperf_tentry.log' skip rls_to_ignore using (2.0):(column(1)/1000):(0):('LowRTT') \
, '<grep -P -o "(?<=K/)(\d*)(?= us )" llfmt_noqueue_busy_wait/iperf_tentry.log' skip rls_to_ignore using (3.0):(column(1)/1000):(0):('LLMT')
