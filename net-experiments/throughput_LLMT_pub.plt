...
set output "Throughputs_LLMT_pub.pdf"
...
, '<grep -v -e "^\[SUM\]" -e "0\.0000-[1-9][0-9]"  srtt_min_busy_wait/iperf_tentry.log |  grep -P -o "( \d+\.\d+)(?= Mbits/sec )"' skip rls_to_ignore using (2.0):(column(1)):(0):('LowRTT') notitle \
, '<grep -v -e "^\[SUM\]" -e "0\.0000-[1-9][0-9]"  afmt_fl/iperf_tentry.log |  grep -P -o "( \d+\.\d+)(?= Mbits/sec )"' skip rls_to_ignore using (1.0):(column(1)):(0):('AFMT') notitle \
, '<grep -v -e "^\[SUM\]" -e "0\.0000-[1-9][0-9]"  llfmt_noqueue_busy_wait/iperf_tentry.log |  grep -P -o "( \d+\.\d+)(?= Mbits/sec )"' skip rls_to_ignore using (3.0):(column(1)):(0):('LLMT') notitle \
